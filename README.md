...
2. 支持字体、字号、文字颜色、背景色、粗体、斜体、下划线，默认字体为思源黑体。
3. 支持字间距、行高、描边颜色、描边宽度、描边位置。
4. 支持文本水平对齐、垂直对齐。
5. 支持编辑区宽度、高度、四向内边距设置。
6. 支持多行预览和单行预览两种模式。
7. 多行模式支持按整行分页的静态翻页和方向翻页动画，文字不会跨页，且每页都会独立应用垂直对齐。
...
插件名为 `hd-text-editor`，构建产物默认发布到 `https://192.168.90.222:66/`。
...
4. 编辑区、预览区、切图区通过“同一份源 HTML + 同一组盒模型参数 + 同一份分页结果”建立联系。
...
编辑区当前使用“滚动容器 + 内层可编辑内容”的结构，`Vertical align` 作用在文字内容本身，而不是移动整个编辑卡片，因此在单页内容场景下可以和预览区、切图区保持一致的垂直分布。

默认字体链路现在也统一收敛到思源黑体：`editor`、`preview-viewport` 和 `cut-preview-item` 都会共享同一套思源黑体字体栈，避免初始文本、预览文本和 Canvas 切图分别落到不同默认字体。
...
多行预览现在采用“连续流测量 + 整行分页 + 逐页垂直对齐”的方案：

1. 用一份完整 HTML 渲染连续流内容
2. 隐藏测量层逐字符提取真实矩形，并归并成真实行盒
3. 按 `可用内容高 = editor height - paddingTop - paddingBottom` 把整行装入每一页
4. 每一页根据 `Vertical align` 重新计算自己的纵向偏移
5. 预览页只裁切属于当前页的整行区间，而不是按固定页高硬切

好处是：

1. 文字不会跨页
2. 每一页都会重复使用同一套 padding 和垂直对齐规则
3. 预览分页与切图分页完全同源
...
3. 获取每个字符的真实位置和真实样式
4. 把字符归并成真实行盒，并按和预览完全一致的规则分页
5. 仅绘制当前页所属的整行字形集合
6. 用 Canvas 重绘成一页一张 PNG
...
6. 多行分页的判断必须基于同源内容和同源盒模型，且预览与切图必须共用同一份“整行分页”结果，不要拆成两套不同排版逻辑。
...
4. 单行内容总宽度最多保留 `65536px`，单张 PNG 切片宽度最多 `8192px`，这是为了控制内存和浏览器画布能力。
5. 项目目前没有撤销/重做历史栈。

...
定义在 `PreviewPanel.vue` 内，是多行模式下的“分页布局缓存”。

它不是简单的页数统计，而是已经完成“整行分页”后的页数组。每一页都包含：
...
多行预览当前采用“先整行布局，再逐页分页”的方式：

1. 先把富文本 HTML 拆成带样式的字符 token
2. 对英文和数字优先按连续词组测宽，再按内容宽度计算整行布局
3. 再按“整行不可跨页”的规则分页
4. 每一页都单独应用当前 `Vertical align`
5. 预览层只渲染本页对应的行集合

好处是：

1. 文字不会在页边界被截断
2. 英文和数字的断行位置更接近编辑区浏览器原生排版
3. 每一页都能独立做顶部 / 居中 / 底部停靠
4. 预览页和切图片页使用同一份分页结果
...
多行切图当前不再按“整块内容 + 固定页高”去硬裁，而是：

1. 直接复用多行分页得到的 `multilinePages`
2. 每一页只渲染本页自己的行集合
3. 每一页重新应用当前 padding 和 `Vertical align`
4. 再用 Canvas 逐页绘制成 PNG
...
为了避免斜体和高亮在 Canvas 中表现成“被切碎的字符块”，切图阶段不是简单按单字符逐个 `fillText`，而是：

1. 先把一行里的连续同样式字符合并成 run
2. 英文和数字在布局阶段优先按连续词组测宽，减少预览与编辑区的断行偏差
3. 高亮背景按 run 一次性绘制
4. 当字间距为 `0` 时，斜体、描边、填充按整段 run 一次性绘制
5. 只有在必须逐字符绘制的场景下，才退回单字符渲染

这样做的目的，是尽量避免：

1. 斜体字符看起来一段一段断开
2. 高亮背景按字符一格一格分裂
3. 描边和阴影在连续文字上出现不自然的断点
...
1. 预览配置为 `multiline`
2. 先按内容宽度完成整行布局
3. 再按“整行不可跨页”规则生成 `multilinePages`
4. 如果页数大于 1，则启动自动翻页计时器
5. 到达 `pageStaySeconds` 后调用 `goToPage(nextPage)`
6. 如果方向是 `static`
   - 直接切页
7. 如果方向不是 `static`
   - 构建双页层
...
| 页数上限 | `10 页` |
| 超过上限 | 后续页不进入预览结果 |
| 分页规则 | 先整行布局，再分页，文字不可跨页 |
| 垂直对齐 | 每一页单独应用 `Vertical align` |
...
6. 多行分页必须先完成整行布局，再分页，不能直接按固定高度硬裁内容。
7. 多行预览和多行切图必须共用同一份分页结果，否则页边界会不一致。
8. Canvas 绘制连续同样式文本时，优先按 run 成组绘制，避免斜体和高亮出现分裂。
9. 单行模式的宽度和速度必须做数值钳制，否则极端值会把渲染和导出拖垮。
